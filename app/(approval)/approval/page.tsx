...
export default function ApprovalPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-sm border border-slate-300">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold text-slate-900">Pusat Persetujuan (Approval Center)</h1>
            <span className="px-2.5 py-0.5 rounded-sm bg-amber-100 text-amber-900 border border-amber-300 text-xs font-bold">
      ...
      {loading ? (
        ...
        <div className="py-16 text-center bg-white rounded-sm border border-dashed border-slate-300 p-8">
        ...
        <div className="space-y-3">
          {requests.map((req) => (
            <div
              key={req.id}
              className="bg-white rounded-sm border border-slate-300 border-l-[3px] border-l-amber-500 p-5 space-y-4"
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200 pb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-sm bg-slate-100 text-slate-700 font-bold flex items-center justify-center text-[11px] text-center leading-tight">
                    #{req.noPengajuan}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="px-2.5 py-1 rounded-sm text-[11px] font-bold uppercase bg-slate-100 text-slate-700 border border-slate-200">
                    Tipe: {req.tipe}
                  </span>
                </div>
              </div>
              ...
              {req.catatan && (
                <div className="p-3 bg-amber-50 rounded-sm border border-amber-300 text-xs">
              ...
              <div>
                ...
                <div className="divide-y divide-slate-200 border border-slate-300 rounded-sm overflow-hidden text-xs">
              ...
              <div className="pt-2 flex justify-end gap-3">
                <button
                  ...
                  className="bg-white hover:bg-rose-50 text-rose-700 font-bold text-xs px-4 py-2 rounded-sm border border-rose-300 transition-all cursor-pointer"
                >
                  Tolak Pengajuan
                </button>
                ...
                <button
                  ...
                  className="bg-brand-700 hover:bg-brand-800 text-white font-bold text-xs px-5 py-2 rounded-sm transition-all cursor-pointer"
                >
                  Setujui Pengajuan ✓
                </button>
              </div>
      ...
      {rejectingReq && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-white rounded-sm p-6 w-full max-w-md border border-slate-300">
      ...
      {approvingReq && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-white rounded-sm p-6 w-full max-w-md border border-slate-300">
            ...
            <div className="divide-y divide-slate-100 border border-slate-300 rounded-md overflow-hidden text-xs mb-4">
